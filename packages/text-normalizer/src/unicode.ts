...
/**
 * Совместимая нормализация Unicode (NFKC): полноширинные символы (`ＩＰＨＯＮＥ`) и
 * идеографический пробел приводятся к обычным ASCII-эквивалентам.
 *
 * Форма именно NFKC, а не NFKD. Совместимая декомпозиция (NFKD) без последующей
 * композиции разбивает кириллические "й" и "ё" на базовую букву и комбинирующий
 * диакритик (U+0306, U+0308): строка "айфон" перестаёт быть равна себе по числу
 * code point'ов ДО словарных сопоставлений в конвейере (`expandSynonyms`,
 * `correctKeyboardLayout`, `transliterateCyrillic` ищут точное совпадение строки),
 * а сам диакритик проходит через остальные шаги как неизвестный символ и оседает
 * артефактом в нормализованном выводе. NFKC выполняет ту же совместимую
 * декомпозицию для приведения полноширинных символов, но затем канонически
 * компонует результат обратно — "и" + U+0306 снова становится "й".
 */
