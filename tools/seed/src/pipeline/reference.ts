 ...
 * Файл ОТСУТСТВУЕТ на момент реализации агента 4 (не в объёме этого агента); вопрос 13 о том,
 * кто формирует выборку, решён (docs/09-decisions.md, ADR-013, дополнение "вопрос 13 закрыт" —
 * агент 5.4 по вендорским страницам, формат — ровно `parseReferenceFile` ниже). Этот модуль
 * реализует МЕХАНИЗМ и корректно работает и на пустом эталоне (сверка просто не даёт покрытия),
 * и на заполненном — без изменения кода, когда файл появится на том же месте.
 */
