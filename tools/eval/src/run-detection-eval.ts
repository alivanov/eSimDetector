 ...
 * (125 записей, все девять групп, docs/08 §8.4) через `POST /api/v1/detect` РАБОТАЮЩЕГО контура
...
export async function runDetectionEval(
  ...
  // Последовательно, с паузой (`lib/pace.ts`) — не «залпом» `Promise.all`: ~125 запросов без пауз
