...
Парсер намеренно устойчив к типовым дефектам выгрузок языковых моделей: символ BOM, обёртка в блок кода Markdown, пояснительный текст до и после таблицы, отсутствие строки заголовка, разделитель `;` вместо `,`, `Да`/`Нет`/`true`/`1` вместо `yes`/`no`, неразрывные пробелы, обрыв строки на середине, повторный заголовок в середине файла, сокращение перечня многоточием. В поле `model_codes` канонический разделитель кодов — `|` ([приложение А](./appendix-a-llm-csv-request.md) §А.2), но фактические выгрузки часто ставят `;` (тот же символ, что разделяет пары в `esim_conditions`); конвейер принимает оба (`tools/seed/src/domain/validate-row.ts`). Иначе склейка вроде `ADY-AL00;ADY-LX9` целиком уходит в `CODE_PATTERN_INVALID`, и живой `Sec-CH-UA-Model` не находится в справочнике. Все отклонения фиксируются в отчёте, а не приводят к падению.
...
**Фактическое состояние (этап 5.7, docs/09-decisions.md ADR-035).** Данные заведены — 10 связок `verified` (китайские окончания Samsung/Huawei/Honor, суффиксы США Samsung, глобальная схема Xiaomi, докс/appendix-a §А.10.5), разбор и резолюция по точному совпадению реализованы как самостоятельный, покрытый тестами модуль (`tools/seed/src/domain/code-suffixes.ts`, две функции — `resolveVerifiedRegion` и дискриминированный `resolveSuffixOutcome`). **Модуль НЕ подключён к резолюции статуса устройства** ни в `tools/seed`, ни в `apps/api/src/modules/detection`: сама таблица данных не может автоматически разрешить ни одного `conditional` до тех пор, пока следующий агент, работающий с `detection`/`matching`, не вызовет `resolveSuffixOutcome` при разборе `Sec-CH-UA-Model` и не передаст результат в `EsimResolutionContext.region` (docs/09 ADR-031, тот же параметр, что уже принимает ответ пользователя на адресный вопрос). До этого подключения региональные условия Android продолжают уходить в уточнение, как и раньше — новых ложных ответов эта таблица сама по себе не создаёт (ADR-028 п.4, ADR-035 п.3: неподтверждённый суффикс не резолвится в отрицательный ответ), но и не устраняет уточнение автоматически.
...
- **В CI — реальная загрузка и сверка, не только `--dry-run`.** Конвейер (docs/08 §8.7) поднимает сервис-контейнер MongoDB, выполняет `pnpm seed load` и `pnpm seed verify` (инварианты §5.8 и сверка с `catalog.reference.json`, код возврата 1 при нарушениях). Режим `--dry-run` остаётся для локальной проверки отчёта без записи в базу.
...
- **Наполнение в демонстрационном контуре — ручной шаг, а не часть `docker compose up` (docs/09-decisions.md ADR-042/ADR-047 п.10; docs/07-integration.md §7.6).** Этот пункт до этапа 8 утверждал обратное («загрузка выполняется отдельным разовым шагом ДО старта API, поэтому `docker compose up` даёт готовый к работе сервис без ручных действий») — прямое противоречие соседнему абзацу того же раздела («Автоматический запуск в демонстрационном контуре» здесь означало «до старта API», но фактический `docker-compose.yml` не содержит шага загрузки вовсе, сервис `api` поднимается с пустым справочником) и ADR-015/ADR-042, которые прямо это фиксируют. Фактическая последовательность (docs/07 §7.6): `docker compose up -d` → `MONGODB_URI=... pnpm seed load` → `MONGODB_URI=... pnpm seed rebuild-signatures` (вторая команда обязательна — без неё ветка iOS теряет сужение по геометрии экрана) → `docker compose restart api` (или `POST /api/v1/admin/catalog/reload` при настроенном `ADMIN_TOKEN`). До выполнения обоих шагов `GET /api/v1/catalog/meta` отвечает `deviceCount: 0`.
- **Связь с модерацией (этап 7, docs/15-moderation.md, docs/09-decisions.md ADR-043).** `pnpm seed load` дополнительно заводит задачи очереди модерации `csv_quarantine` (из `result.quarantine`, включая нарушения инвариантов §5.8, найденные после построения) и `source_disagreement` (записи, разрешённые правилом осторожности шага 5) — напрямую в `moderation_tasks`, той же дедупликацией `kind`+`key`, что и у записи через API. `pnpm seed rebuild-signatures` учитывает `catalog_overrides` перед пересборкой `screen_signatures` (§14.5а ниже, тот же принцип, что не даёт повторному импорту стирать решения модератора в `devices`). `pnpm seed export-overrides` выгружает не только патчи (`data/catalog/overrides/`), но и устройства, созданные модератором целиком (`provenance.source` вида `moderator:<логин>`) — в `data/catalog/curated/moderator-<id>.json`, тот же каталог, что и вендорское курируемое ядро.
