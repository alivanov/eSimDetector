...
| `moderation_tasks`  | `resolvedAt` (частичный TTL, только решённые/отклонённые задачи) | Ограничение срока хранения (docs/09-decisions.md ADR-047 п.7) — открытая задача (`resolvedAt: null`) не подпадает под фильтр и не удаляется по возрасту |
