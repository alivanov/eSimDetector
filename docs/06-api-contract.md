...
Раздел описывает интерфейс взаимодействия для критерия К4. Машиночитаемая спецификация OpenAPI 3.0.0 генерируется из кода (`@nestjs/swagger` без `.setOpenAPIVersion()`) и доступна на `/api/docs` (Swagger UI) и `/api/docs-json`.
...
| Ограничение частоты   | По IP и по ключу API; при превышении — `429` с `Retry-After`. Запрос с валидным `X-Admin-Token` (совпал с непустым `ADMIN_TOKEN`) в квоту не входит (ADR-049) |
...
Поле `device.brand` в форме `DeviceSummary` (`toDeviceSummary`) — это отображаемый заголовок `brandTitle` (в примере `"Samsung"`), а не слог бренда; слог (`"samsung"`) отдаёт `DeviceCard.brand` на `GET /devices/{id}` (§6.4) рядом с отдельным `brandTitle`.
...
| `GET /metrics`                               | Метрики в формате Prometheus (не реализовано — вне объёма этапов 1–8, см. передачу)             |

**Реализовано этапом 8 (docs/09-decisions.md ADR-047 п.11): `GET /api/v1/devices/{id}`, `GET /api/v1/devices?brand=&platform=&page=`, `GET /api/v1/brands`.** Все три — публичные (без ключа, как и остальные эндпоинты §6.4), читают только уже прогретый снимок `CatalogService` (`.cursor/rules/api-boundaries.mdc`) и ограничены записями `status: "active"`. `GET /devices/{id}` отдаёт форму `DeviceCard` (`apps/api/src/common/response/device-card.ts`) — то же, что и запись справочника, кроме `provenance` (внутренний след импорта, не часть публичного контракта). `GET /devices` и `GET /brands` реализованы в `apps/api/src/modules/matching` (тот же модуль, что и `/devices/search`/`/devices/suggest`) — не заводят отдельный модуль ради трёх маршрутов, которые читают тот же `CatalogService`.

**Не реализован и НЕ БУДЕТ реализован в объёме проекта: `POST /api/v1/clarify` (docs/09-decisions.md ADR-047 п.12).** Контракт `{ requestId, deviceId } | { requestId, answer } → итоговый результат` предполагает, что сервер способен восстановить исходные сигналы запроса `requestId` из своего состояния — но `/detect`/`/devices/search` спроектированы идемпотентными БЕЗ состояния сессии на сервере (ADR-024, ADR-031 п.3: «клиент обязан прислать сигналы и ответ заново с каждым запросом»), и это решение принято сознательно ради масштабирования (несколько экземпляров API без общего хранилища сессий, docs/07 §7.9) и потому, что регион — явный ответ пользователя, а не что-либо, что сервер обязан «помнить». Заводить хранилище сессий ради одного эндпоинта означало бы пересмотр этого архитектурного решения, а не работу над контрактом/интеграцией (вне объёма этапа 8, AGENTS.md: «не переписывай алгоритмы detection/matching»). Интерфейс (`apps/widget/src`, ADR-039) не нуждается в этом эндпоинте буквально: выбор кандидата уточнения (`clarification.kind === 'choose_candidate'`, а также выбор из `candidates[]` при определённом статусе группы) выполняет `GET /api/v1/devices/{id}` по `id` выбранного варианта и строит экран результата по карточке (после этапа 6 сдачи п.1–2 повторный `POST /devices/search` по короткой подписи вроде «Apple iPhone XS» снова уходит в `choose_candidate` из‑за близких оценок XS/XS Max — поэтому обход через подпись заменён на запрос по id, см. ADR-047 п.11); ответ на региональный вопрос — повторный `POST /api/v1/detect`/`/devices/search` с `context.region`/`region`, заполненным сигналами, которые клиент уже собрал на первом шаге. Строка `POST /api/v1/clarify` удалена из таблицы §6.4 ниже как нереализуемая в рамках принятой архитектуры, а не как забытый пункт объёма.
...
**Стенд оценки из `/admin` (ADR-049).** Группа за `X-Admin-Token`: `POST /api/v1/admin/eval/runs` (старт; `409 EVAL_RUN_IN_PROGRESS`, если уже идёт прогон), `GET /api/v1/admin/eval/runs`, `GET /api/v1/admin/eval/runs/{id}`, `GET /api/v1/admin/eval/runs/{id}/report` (Markdown). Прогон асинхронный, запись в коллекцию `eval_runs`, отчёт в MongoDB. Полная таблица — docs/15-moderation.md §15.8.
...
| `UNAUTHORIZED`         | 401  | Отсутствует или неверен `X-Api-Key` / `X-Admin-Token` в защищённом режиме                                                     | Проверить ключ                           |
| `DEVICE_NOT_FOUND`     | 404  | Запрошен неизвестный `deviceId`                                                                                               | Проверить идентификатор                  |
| `TASK_NOT_FOUND`       | 404  | Задача модерации (`/api/v1/admin/moderation/tasks/{id}`) неизвестна либо `{id}` не является 24-символьным hex-идентификатором | Проверить идентификатор задачи в очереди |
| `EVAL_RUN_NOT_FOUND`   | 404  | Прогон стенда оценки (`/api/v1/admin/eval/runs/{id}`) неизвестен либо отчёт отсутствует                                       | Проверить идентификатор прогона          |
| `EVAL_RUN_IN_PROGRESS` | 409  | Повторный `POST /api/v1/admin/eval/runs`, пока уже идёт прогон; либо скачивание отчёта до завершения                          | Дождаться завершения прогона             |
| `PAYLOAD_TOO_LARGE`    | 413  | Тело запроса превышает лимит парсера Express (`body-parser`); без этого кода сырой 413 уходил бы в `INTERNAL_ERROR` 500       | Уменьшить тело запроса                   |
...
Команды проверены на поднятом демонстрационном контуре (`docker compose`, справочник 1057 устройств). Ожидаемые фрагменты ответа — фактические, не иллюстративные.

```bash
# Определение по названию
# → status: "clarification_required", query.normalized: "iphone 13",
#   reasons: MATCH_EXACT_ALIAS, DECISION_BELOW_THRESHOLD; device: null
curl -s 'http://localhost:3000/api/v1/devices/search?q=айфон%2013' | jq

# Устойчивость к неверной раскладке («samsung 23», набрано в русской раскладке)
# → query: { "raw": "Ыфьыгтп 23", "normalized": "samsung 23" }
#   (строка «Ыфьыгте Ы23» раскладкой не является: даёт normalized "yfygte y 23")
