...
Когда статус группы кандидатов не согласован, но причина расхождения ОДНА и ТА ЖЕ у всех кандидатов — одно и то же нерешённое условие `esim.conditions` (докс/05 §5.4) с буквально совпадающим `clarifyingQuestion` — сервис задаёт этот вопрос напрямую вместо выбора из списка (докс/03 §3.7, п.2; этап 5.3а):
...
```json
{
  ...
  "reasons": [
    { "code": "SCREEN_SIGNATURE_MATCHED", "detail": "393x852@3" },
    { "code": "CANDIDATES_DISAGREE_ON_ESIM", "detail": "4 кандидат(ов), статус расходится" }
  ],
  "clarification": {
