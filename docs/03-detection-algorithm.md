...
внутреннее представление), `category`/`source` выбираются оператором из допустимых значений
перечисления, поле `expected` предзаполняется полями ответа (`detection.platform`/`deviceType`,
`status`, `exactModelKnown`, `device?.id`), но интерфейс явно помечает его как черновик, требующий
ручной проверки. Это принципиально: `expected`, списанное с ответа без проверки, превращает
запись в снимок ТЕКУЩЕГО поведения сервиса и лишает выборку способности обнаружить в этом
поведении ошибку — тот же класс проблемы, против которого написаны ADR-013 и ADR-032 п.4 («не
подправлять эталон под выгрузку»), здесь применённый не к справочнику устройств, а к сигналам.
Наполнение самого файла `data/fixtures/signals.golden.json` заготовками со стенда, включая
собственно ручную проверку `expected`, — объём агента, отвечающего за эту выборку (docs/09 ADR-041).
