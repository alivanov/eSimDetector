...
- **Статус большинства групп — «условный», а не «поддерживается»**: версии для континентального Китая идут с двумя физическими nano-SIM и eSIM не имеют, поэтому запись такой модели имеет `esim.support: conditional` с условием по региону (docs/05 §5.4, случай 1). Регион не входит в собираемые сигналы (§3.2) вовсе, поэтому по одной сигнатуре экрана такая группа честно уходит в уточнение вместо догадки — следствие, разобранное в §3.9.
...
- **Следствие появления курируемого ядра Apple (этап 5.3, ADR-030), которое стоит знать до демонстрации:** регион не входит в собираемые сигналы (§3.2), поэтому запись с условием по региону (`esim.conditions[scope="region"]`) на ветке iOS всегда резолвится в уточнение — а условие по КНР есть у большинства моделей от iPhone XR до 17 Pro Max. Автоматический однозначный ответ по одной сигнатуре экрана остаётся у групп без региональных условий: линейка Plus до появления eSIM (`не поддерживается`), SE 2/3 в сочетании с iOS 17+ (`поддерживается`), iPhone Air (`поддерживается`, единственный кандидат сигнатуры). Для остальных групп сервис показывает список моделей — что не является ложным результатом, но и не является ответом. Адресный вопрос «куплен ли iPhone в Китае?» у записи уже заполнен (`esim.clarifyingQuestion`), однако ветка группы iOS его не показывает: `buildIosClarification` формирует выбор из кандидатов. Закрытие этого пробела — объём агента, отвечающего за сценарий уточнения, а не этапа данных.
