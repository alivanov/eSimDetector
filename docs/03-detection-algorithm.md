...
## 3.10. Стенд отладки `/debug` как канал сбора сигналов (этап 6.4)

**Исправление висячей ссылки.** `tools/eval/src/signals-golden.ts` (агент 5.7) ссылался на этот
раздел («страница отладки, docs/03 §3.10») до того, как он был написан — на момент агента 5.7
раздела не существовало, а сама страница появилась только этапом интерфейса (docs/11 §11.2,
этап 4/агент 6). Раздел заводится этим агентом (6.4), чтобы ссылка не осталась висячей: другой
стороной несоответствия мог быть либо этот раздел, либо комментарий кода — фактическая проверка
(чтение обоих файлов) показала, что неверна ссылка в документе (её не существовало), а не
комментарий кода (он был написан заранее, в ожидании этого раздела, и корректен по существу).

**Назначение.** Страница `http://localhost:8080/debug` (`apps/web/src/debug/DebugPage.tsx`,
docs/07-integration.md §7.6, ADR-010) — практический инструмент разбора расхождений (§3.6 выше:
массив `reasons[]` как инструмент отладки) и, наравне с виджетом на реальном устройстве, один из
двух практических каналов сбора сигналов для эталонной выборки `signals.golden.json`
(docs/08-testing-and-quality.md §8.4, ADR-037): страница показывает собранные сигналы этого
браузера как редактируемый JSON, принимает произвольный набор сигналов или произвольный
`userAgent`, отправляет их на `POST /api/v1/detect` и показывает ответ сервиса целиком — статус,
уверенность, весь блок `detection`, `device`, `candidates[]`, `reasons[]`, `clarification` и
`presentation` — без каких-либо собственных заключений о статусе eSIM (ограничение объёма
этапа 6.4, то же правило, что и AGENTS.md, предметное правило 3: список кандидатов iOS показан
как есть, без «наиболее вероятной» модели).

**Формирование записи выборки — заготовка, не готовая запись.** Кнопка «Скопировать запись для
signals.golden.json» строит черновик формата `SignalsGoldenEntry` (`tools/eval/src/signals-golden.
ts`) из последнего ответа `/detect`: поле `signals` — ровно тело, отправленное на эндпоинт (а не
внутреннее представление). `category` предлагается по отправленным сигналам (`suggestGoldenCategory`
в `apps/web/src/debug/golden-export.ts`: WebView, эмуляция, планшет, iPhone, заполненный
`uaData.model`, браузер не Chrome/Safari, Android без UA-CH, десктоп) — не по `detection.method`
и не по статусу ответа, иначе выборка повторит ошибку сервиса. Подсказка под полем объясняет,
почему выбрана корзина; оператор может сменить значение. `source` подставляется только при явной
эмуляции (`browser-emulation`), иначе остаётся выбор оператора. Поле `expected` предзаполняется
полями ответа (`detection.platform`/`deviceType`, `status`, `exactModelKnown`, `device?.id`), но
интерфейс явно помечает его как черновик, требующий ручной проверки. Это принципиально:
`expected`, списанное с ответа без проверки, превращает запись в снимок ТЕКУЩЕГО поведения
сервиса и лишает выборку способности обнаружить в этом поведении ошибку — тот же класс проблемы,
против которого написаны ADR-013 и ADR-032 п.4 («не подправлять эталон под выгрузку»), здесь
применённый не к справочнику устройств, а к сигналам. Наполнение самого файла
`data/fixtures/signals.golden.json` заготовками со стенда, включая собственно ручную проверку
`expected`, — объём агента, отвечающего за эту выборку (docs/09 ADR-041).
