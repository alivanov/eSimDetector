...
**Классификация исхода К2 (`resolveActualOutcome`, `tools/eval`).** `clarification_required` с непустым `matches[]` (выбор кандидата) — всегда `clarification`, даже при ошибочно заполненном `device` (иначе лидер группы считался бы верным `match`). Непустой `device` без списка кандидатов — `match` (модель названа; статус eSIM может ждать ответа на условие). `supported`/`not_supported` при `device: null` — ответ группы эквивалентности без точной модели (ADR-002) и тоже `match`; ложным считается только уверенно названное неверное устройство (`deviceId !== null` и не совпал с ожиданием).
