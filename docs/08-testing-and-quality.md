...
**Фактическое состояние выборки (агент 6.6, docs/09-decisions.md ADR-042; дополнение полевым сбором 2026-08-25).** `data/fixtures/signals.golden.json` наполнен — **125 записей**, все девять обязательных групп непусты:
...
| `android-vendor-ua-ch` | 28      | 26 `public-ua-database` (составные) + 2 `real-device` (Pixel 9; Galaxy S25 Ultra)           |
