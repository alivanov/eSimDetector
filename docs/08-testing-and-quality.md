...
**Фактическое состояние выборки (агент 6.6, docs/09-decisions.md ADR-042).** `data/fixtures/signals.golden.json` наполнен — **121 запись**, все девять обязательных групп непусты:

| Группа                 | Записей | Канал сбора                                     |
| ---------------------- | ------- | ----------------------------------------------- |
| `iphone-generations`   | 35      | `browser-emulation`                             |
| `android-vendor-ua-ch` | 26      | `public-ua-database` (составные записи)         |
...
По каналу сбора: 80 записей `browser-emulation`, 41 запись `public-ua-database`, **0 записей `real-device`** — у команды нет физических устройств для проверки (уточнено с пользователем, зафиксировано прямо, а не по умолчанию). Доступны только два канала сбора из трёх, описанных прозой выше: `browser-emulation` — реальные дескрипторы устройств Playwright (`@playwright/test`, `devices`), прогнанные через настоящую страницу `/debug` работающего контура (сигналы сняты собственным сборщиком `packages/signals-collector`, а не набраны руками); `public-ua-database` — реальные строки User-Agent из открытых источников (официальная документация Chrome/Samsung/Mozilla, независимые каталоги строк User-Agent, тестовый корпус `ua-parser-js`), перечисленные для каждой записи в `notes` со ссылкой. Группа `android-vendor-ua-ch` (Android с реальным заполненным `Sec-CH-UA-Model`) эмуляцией недостижима в принципе (см. ниже) — все 26 её записей составные: реальная строка редуцированного User-Agent Chrome для Android (официальный пример MDN, «User-Agent reduction») плюс сервисный код/маркетинговое название из курируемого ядра справочника (`data/catalog/curated/google-pixel.json`, `data/catalog/code-suffixes.json`, уровень `verified`, ADR-026/ADR-035) — в `notes` каждой такой записи явно написано «СОСТАВНАЯ запись, а не наблюдённая целиком».

`expected` каждой записи выведен из правил docs/03 (§3.2а классификация типа, §3.5 шаг 1 правило по версии iOS, §3.8 обнаружение эмуляции) и данных справочника **до** обращения к API, а не списан с ответа сервиса (ADR-042, тот же принцип, что ADR-013/ADR-032 п.4 применяют к справочнику устройств). Разовая сверка всех 121 записи с фактическим ответом работающего контура (`docker compose up -d`, прямые запросы `POST /api/v1/detect`) дала **121/121 (100%) совпадений** после разбора четырёх найденных на первом прогоне расхождений — все четыре оказались ошибками вывода ожидания самого агента (не учтён формат UA с полем локали, ломающий устаревший разбор модели; не учтено, что гейт достоверности данных блокирует только статус, а не факт распознавания устройства; неверно посчитан минимальный CSS-размер экрана для граничного случая планшета), исправлены в самих записях с указанием причины в `notes`; ни одно расхождение не было отнесено к дефекту модуля `detection` — подробный разбор и числа до/после исправления — отчёт этапа 6.6 (передача агенту 7).

**Границы достоверности (обязательная оговорка).** Выборка из эмуляции браузера и открытых баз User-Agent измеряет **согласованность алгоритма с собственными правилами**, а не точность определения на живых устройствах — «121 запись» не означает «проверено на 121 устройстве». Эмуляцией и открытыми базами принципиально недостижимо:

- **Настоящий `Sec-CH-UA-Model` от живого Android-браузера.** `navigator.userAgentData.getHighEntropyValues(['model'])` в Chromium/Playwright возвращает пустую строку независимо от эмулируемого устройства (проверено эмпирически на всех записях `android-no-ua-ch`, полученных через `browser-emulation`, — реальный собранный сигнал `uaData.model === ""` у каждой из них) — единственный практический путь получить модель в эмуляции — устаревший разбор User-Agent, а не основной сигнал ветки Android (docs/03 §3.4), который эмуляция не воспроизводит вовсе. Отсюда прямое следствие: все 26 записей `android-vendor-ua-ch` — составные, а не наблюдённые целиком (см. выше).
- **Фактическое поведение Safari на iOS.** Эмуляция iOS-профилей в этом проекте — Chromium с подменённым User-Agent (`defaultBrowserType: 'chromium'`, тот же приём, что и `apps/e2e`), а не настоящий WebKit: `deviceMemory` в настоящем Safari отсутствует вовсе (в собранных записях он присутствует, потому что его действительно передаёт Chromium), строка рендерера WebGL иная, а режим «Увеличенный» (Display Zoom, docs/03 §3.5 шаг 3) не воспроизводится ни одним профилем.
