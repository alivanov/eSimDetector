...
**Фактическое состояние выборки (агент 6.6, docs/09-decisions.md ADR-042; дополнение полевым сбором 2026-08-25).** `data/fixtures/signals.golden.json` наполнен — **132 записи**, все девять обязательных групп непусты:
...
| `android-vendor-ua-ch` | 33      | 26 `public-ua-database` (составные) + 7 `real-device` (Pixel 9; S25 Ultra; S24 SM-S9210; Note 13 Pro; Note 9 Pro; Galaxy A21s SM-A217F; Galaxy A10 SM-A105F) |
