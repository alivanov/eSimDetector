...
| `matching`          | Оркестрация конвейера (вызов `text-normalizer` и `fuzzy-matcher`), отбор кандидатов по индексам справочника, применение решения (§4.7) и формирование ответа. Слотовый разбор и распознавание сервисных кодов сюда не входят — они часть `text-normalizer` (ADR-019) и тестируются там                     | ≥ 90%            |
