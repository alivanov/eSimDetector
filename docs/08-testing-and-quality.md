...
**Фактическое состояние выборки (агент 6.6, docs/09-decisions.md ADR-042; дополнение полевым сбором 2026-08-25).** `data/fixtures/signals.golden.json` наполнен — **123 записи**, все девять обязательных групп непусты:

| Группа                 | Записей | Канал сбора                                                                  |
| ---------------------- | ------- | ---------------------------------------------------------------------------- |
| `iphone-generations`   | 36      | 35 `browser-emulation` + 1 `real-device` (iPhone 14 Pro Max, Яндекс.Браузер) |
