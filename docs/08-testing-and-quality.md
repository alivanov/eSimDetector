...
На полном прогоне конвейера доля расхождений выгрузки с эталоном на пересечении (179 строк) составила **19,0%** (145 совпало, 34 карантинировано кодом `REFERENCE_MISMATCH`) — печатается `pnpm seed import`/`consensus` и `pnpm seed verify`. Часть записей эталона (линейки Z Fold/Z Flip/Note) заведена заранее и станет измеримой только после того, как какой-либо агент импортирует партию 3 — до этого момента пересечение по ним равно нулю, что не является дефектом эталона (docs/14 §14.4 шаг 4 корректно работает и при отсутствии пересечения, `tools/seed/src/pipeline/reference.ts`, докстринг `applyReferenceCheck`). Формат файла и совпадение `id` с реальными идентификаторами справочника покрыты тестом данных `tools/seed/src/pipeline/reference-data.spec.ts` (по образцу `curated-apple.spec.ts`/`curated-pixel.spec.ts`).

**Число не перемерено этапом 5.5.** Партия 3 (складные Samsung и Galaxy Note) этому этапу не передана — `data/catalog/import/` не содержит ни одного нового файла (приложение А §А.6, примечание к таблице) — поэтому пересечение с линейками Z Fold/Z Flip/Note эталона по-прежнему равно нулю, а 19,0% остаётся последним фактически измеренным значением, а не оценкой «до импорта партий агента 5.5», как это было сформулировано ранее. Как только партия 3 (или любая другая партия Samsung) появится в `data/catalog/import/`, следующий по очереди агент, выполняющий импорт, обязан перемерить это число и обновить абзац — использованная здесь оговорка «до импорта агентом 5.5» больше не соответствует факту и не должна читаться как обещание конкретного будущего агента.
