...
**Фактическое состояние выборки (агент 6.6, docs/09-decisions.md ADR-042; дополнение полевым сбором 2026-08-25).** `data/fixtures/signals.golden.json` наполнен — **127 записей**, все девять обязательных групп непусты:
...
| `android-vendor-ua-ch` | 30      | 26 `public-ua-database` (составные) + 4 `real-device` (Pixel 9; S25 Ultra; S24 SM-S9210; Redmi Note 13 Pro) |
