...
**Непустой `unparsed` не смягчает и не ужесточает жёсткие предикаты `rejectCandidate`.** Это отдельный вопрос от таблицы решения выше: `unparsed` (например, испорченный модификатор — §4.10.1, дефект 1) влияет на исход через составляющую `tokenCoverage` в `scoreCandidate`, а не через `checkModifierSet`/`checkGeneration` в `rejectCandidate`. Такое разделение рассматривалось и признано достаточным — подробности и фактическая проверка в §4.10.1, чтобы следующие агенты не открывали этот вопрос заново без контекста.
