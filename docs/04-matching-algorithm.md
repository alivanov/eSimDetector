...
Ыфьыгтп Ы23              (набрано в неверной раскладке)
...
**Исправление раскладки.** Строки вида `Ыфьыгтп` (это `Samsung`, набранный в русской раскладке) распознаются по признаку: строка состоит из кириллицы, но её посимвольное отображение в раскладку QWERTY даёт последовательность, присутствующую в словаре псевдонимов. Проверка выполняется только при неудаче обычного разбора и требует подтверждения по словарю — это исключает ложные срабатывания.
...
**Формирование ответа при эквивалентной группе (`MatchingService`, ADR-002).** Когда `decide` вернул `determined` с несколькими кандидатами (`DECISION_RESOLVED_BY_EQUIVALENCE`), сервис **не** выбирает лидера как точную модель: `device: null`, в презентации `exactModelKnown: false`. Статус `supported`/`not_supported` выдаётся только при согласии всех живых записей группы (`resolveCandidateGroupEsimStatus`); иначе — `clarification_required`. Если у всех кандидатов одно и то же нерешённое условие eSIM (общий `clarifyingQuestion`, docs/03 §3.7 п.2, ADR-045), задаётся этот `answer_question` **без** списка моделей; `choose_candidate` остаётся для расхождения статусов или разных вопросов.
...
**`determined` не выдаётся, если лидер уже запроса.** `rejectCandidate` по-прежнему не исключает Ultra/Pro/11R, когда запрос их не назвал — иначе «galaxy s23» потерял бы варианты для уточнения. Но если после `decide` лидер несёт однобуквенное обозначение или многобуквенный модификатор, которых не было в запросе, статус опускается до `clarification_required` (кандидаты остаются). Иначе отсутствующая базовая модель подменяется Spark 10 Pro / OnePlus 11R / Nokia C02. Исключение: решение получено через `DECISION_RESOLVED_BY_EQUIVALENCE` при **нескольких** кандидатах — это ответ группы с общим статусом eSIM (ADR-002), а не выбор Ultra; популярность часто ставит Ultra выше базы, и без исключения эквивалентность снова превращалась бы в избыточное уточнение. Один кандидат через эквивалентность по-прежнему понижается.
