...
### 4.10.1. Что уже проверено (агент 2.5) и что отложено до появления справочника
...
То, что можно и нужно было проверить уже сейчас, без справочника и без конвейера сопоставления, — это первую половину пайплайна §4.3 (нормализация + слотовый разбор, ADR-019) на реальном словаре `data/catalog/aliases.json` и на реальной эталонной выборке. Это сделано модульным тестом `tools/eval/src/data-files.spec.ts` (не стендом, docs/08-testing-and-quality.md §8.4): для каждой из 362 записей `queries.golden.json` он вызывает `normalizeQuery(query, dict).slots` и сверяет результат с `expectedSlots`, зафиксированным в файле. Это подтверждает, что нормализация и слотовый разбор корректно ведут себя на полном объёме реалистичного ввода, включая раскладку, транслитерацию, синонимы и извлечение атрибутов — то есть именно ту часть конвейера, которая уже реализована.

**Отложено до появления справочника (агент 3+):** сверка `expectedOutcome`/`expectedDeviceId` из `queries.golden.json` — они описывают ожидание ПОЛНОГО конвейера (отбор кандидатов + правило принятия решения) и являются aspirational, а не проверенным сейчас фактом. Как только появятся `MatcherDevice[]` из справочника и модуль `matching`, эти поля станут входом для настоящего стенда `eval:matching` из этого раздела.

**Отдельная находка, зафиксированная агентом 2.5, но не исправленная (не входит в объём этого этапа).** Синонимы `"с23": ["s23"]` и `"с24": ["s24"]` в `data/catalog/aliases.json` практически недостижимы при текущем порядке шагов конвейера `normalizeQuery` (`normalize-query.ts`): `splitLettersAndDigits` выполняется раньше токенизации и `expandSynonyms`, поэтому токен `"с23"` разбивается на `"с"` и `"23"` ДО того, как словарь синонимов успевает его увидеть целиком, — раскрытие в `"s23"` не происходит ни при каком входе. Сам факт не проявляется как ложный ответ (кириллическая `"с"` перед числом просто остаётся неразобранным словесным токеном), поэтому это не нарушение доменных правил AGENTS.md, а дефект данных словаря: правило не работает, но и не создаёт риска. Возможные исправления (не выполнены в этом этапе, решение — за агентом, отвечающим за `text-normalizer`/данные): убрать эти два ключа как мёртвый код словаря, либо — если такое сокращение действительно нужно поддержать — реализовать его иначе (например, отдельным шагом до `splitLettersAndDigits`, для чего потребуется менять пакет `text-normalizer`, что вне ограничений этого этапа).
