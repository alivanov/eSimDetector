...
**Реализация этапа 7 (`apps/web/src/admin`).** Пять перечисленных экранов реализованы как вкладки одной страницы `/admin` (React-состояние, без роутера — тот же приём, что и `/debug`, docs/07 §7.6), под общей формой входа по токену (проверяется реальным запросом к `GET /api/v1/admin/catalog/stats`, а не локальной эвристикой формата строки). Тексты экрана написаны этим агентом и не прошли согласование пользователем одним проходом (ADR-025 п.3, docs/13-branding.md §13.5) — раздел не существовал на момент утверждения §13.5/§13.6, тот же прецедент, что и у технических подписей стенда `/debug`.
