...
| 5.5   | Импорт оставшихся партий выгрузок: флагманы Xiaomi (5), Huawei (8) — единственный источник платформы `harmonyos`, складные Samsung (3), прочие бренды (12)                                                                                                                                                                                                                                        | **заблокировано: файлы партий не предоставлены.** `data/catalog/import/` не содержит ни одного файла партий 3/5/8/12 (приложение А §А.6) — импорт/консенсус/загрузка не выполнялись за отсутствием предмета. Выполнено смежное: найден и исправлен дефект классификации HarmonyOS по User-Agent (docs/09 ADR-024, дополнение этапа 5.5); lint/typecheck/test/build подтверждены проходящими. Партии и связанные пробелы охвата остаются открытыми, в объём 5.6/5.7 не переносятся |
...
| Платформа `harmonyos` — 0 записей                       | 5.5       | **Не закрыто.** Партия 8 (Huawei) — единственный источник этой платформы — этапу 5.5 не передана; ветка `detection` проверена на реальном образце UA (docs/09 ADR-024, дополнение этапа 5.5), но не на данных |
| Флагманы Xiaomi — 0 записей                             | 5.5       | **Не закрыто.** Партия 5 этапу 5.5 не передана                                                                                                                                                                |
| Samsung Galaxy Z Fold, Z Flip, Note — 0 записей         | 5.5       | **Не закрыто.** Партия 3 этапу 5.5 не передана; эталон (`catalog.reference.json`) уже готов принять эти линейки при появлении партии (ADR-032)                                                                |
| Nothing, Motorola, ASUS, Sony, Fairphone, HMD/Nokia     | 5.5       | **Не закрыто.** Партия 12 этапу 5.5 не передана                                                                                                                                                               |
