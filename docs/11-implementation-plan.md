...
- запросы выгрузок по партиям из [приложения А](./appendix-a-llm-csv-request.md) у двух-трёх разных моделей; перед массовым прогоном — проверка пригодности источника на одной партии (§А.8). **Состояние:** собрано десять партий у четырёх источников (§А.6, столбец «Состояние»); что осталось и в каком порядке — §А.9, чего в массиве нет вообще — §А.8.3;
- курируемое ядро: Apple и Google Pixel целиком вручную (перечни короткие, правила детерминированные), затем наиболее распространённые модели Samsung и Xiaomi;
- сопоставление «суффикс сервисного кода → регион продажи» для брендов с региональной зависимостью eSIM: пилот показал, что языковые модели полного набора суффиксов не дают, а без него весь модельный ряд Samsung остаётся в статусе `conditional` и требует уточняющего вопроса там, где ответ выводится из кода автоматически. Порядок работы — партия 16 (§А.10): выгрузка даёт перечень кандидатов, однозначный ответ разрешает только ручная сверка с вендорскими источниками;
- контрольная выборка фактов `catalog.reference.json` — не менее 150 моделей, подтверждённых по вендорским источникам; это одновременно инструмент измерения качества выгрузки;
