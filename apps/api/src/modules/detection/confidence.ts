 ...
 *
 * Явное решение (этап 5.3а, docs/09 ADR-031): наличие ответа пользователя на региональный вопрос
 * (`context.region`) НЕ добавляет отдельный вклад сюда — это не сигнал устройства с частичной
 * надёжностью, а точное значение, разрешающее категорический вывод `esim-rules` полностью;
 * базовая уверенность метода (`androidExactCode`/`iosCandidatesAgree*`) остаётся прежней, меняется
 * только сам статус (`clarification_required` → `supported`/`not_supported`).
 */
