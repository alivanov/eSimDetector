...
const config: Config = {
   ...
   *
   * `moderation` (этап 7, доведено этапом 8 — docs/09-decisions.md ADR-047 п.9) — целевое
   * значение таблицы §8.2 — «≥ 85%»; фактический порог в конфиге — 90% по операторам/строкам/
   * функциям (выше таблицы, с запасом) и 78% по ветвлениям (ниже 85%) по тому же принципу
   * декоратора, что и `detection`/`matching` выше, плюс защитные ветки Mongoose-ориентированного
   * кода (устройство не найдено, документ не проходит разбор схемой при повреждении,
   * `deprecated`-записи в публичном каталоге), часть которых стабильно покрыта тестами, но не
   * все комбинации оправдывают отдельный тест ради самой метрики.
   * Фактическое измерение полного прогона `apps/api` (2026-08-24, этап 6 сдача п.6):
   * statements 97,28 / branches 83,13 / functions 97,08 / lines 97,13 —
